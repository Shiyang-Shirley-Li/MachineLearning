...
train_data = Read.read_data("../Data/train_final.csv", True)
attribute_val_dict = Read.attribute_val
Read.change_numeric_to_binary(train_data, attribute_val_dict)
test_data = Read.read_data("../Data/test_final.csv", False)
Read.change_numeric_to_binary(test_data, attribute_val_dict)
...
def test_predict_result(s, root_node):
    test_predict_r = []
    for i in range(len(s)):
        row_attributes = []
        for key in total_attributes:
            row_attributes.append(s[i][key])
        test_predict_r.append(predict_result(row_attributes, root_node))

    return test_predict_r
...
if __name__ == '__main__':
    ...
    print(test_predict_result(test_data, tree.root))
